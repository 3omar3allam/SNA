...
class Profile extends Page {
    ...
    private HBox set_timeline(User user){
        ...
        if(this.online_user == user) {
            ...
            Label lbl_mutual = new Label();

            GridPane.setConstraints(lbl_name,0,0,2,1);
            GridPane.setConstraints(lbl_gender_and_age,0,1);
            GridPane.setConstraints(lbl_mutual,1,1);
            GridPane.setConstraints(lbl_no_friends,0,2);
            GridPane.setConstraints(lbl_no_groups,1,2);
            info.getChildren().addAll(lbl_name,lbl_gender_and_age,lbl_mutual,lbl_no_friends,lbl_no_groups);

            if(this.online_user != null && this.online_user.isFriend(user)){
                int friend_index = userNameBinarySearch(online_user.getFriends(),0,online_user.getFriends().size(),user.getUserName());
                int mutual = online_user.getMutualFriendsNo().get(friend_index);
                if (mutual==1)lbl_mutual.setText("one friend in common");
                else lbl_mutual.setText(String.format("%d friends in common",mutual));
                ...
                if(online_user == null) lbl_mutual.setVisible(false);
                else{
                    int mutual = MutualFriendsNo(online_user,user);
                    if (mutual==1)lbl_mutual.setText("one friend in common");
                    else lbl_mutual.setText(String.format("%d friends in common",mutual));
                }
    ...
    private VBox list_item(Post post,User user,ObservableList array_of_posts){
        ...
        Text txt_post = new Text(post.toString());
        content.setStyle("-fx-padding: 10px;-fx-min-width: 480px;-fx-max-width:480;-fx-max-height: 200;-fx-min-height: 50px");
        txt_post.wrappingWidthProperty().bind(content.widthProperty());
        content.setFitToWidth(true);
        content.setContent(txt_post);
    ...
    private VBox set_friends_and_groups(User user){
        ...
        if(user.getNoFriends() != 0) {
            ...
            for (User friend : user.getFriends()) {
                int mutual = MutualFriendsNo(this.online_user,friend);
                Hyperlink link = new Hyperlink();
                link.setText(String.format("%s\n(%d mutual)",friend.getName(),mutual));
